...
import com.modou.core.enums.SignType;
import com.modou.core.enums.TradeType;
import com.modou.core.kit.IpKit;
import com.modou.demo.entity.H5SceneInfo;
import com.modou.demo.entity.WxPayBean;
import com.modou.demo.vo.AjaxResult;
import com.modou.wxpay.WxPayApi;
import com.modou.wxpay.WxPayApiConfig;
import com.modou.wxpay.WxPayApiConfigKit;
import com.modou.wxpay.model.*;
...
public class WxPayController extends AbstractWxPayApiController {
    ...
    @ResponseBody
    public String index() {
        log.info("欢迎使用 modou 中的微信支付 -by 金钰鑫  <br/><br>");
        log.info(wxPayBean.toString());
        return ("欢迎使用 modou 中的微信支付 -by 金钰鑫  <br/><br>");
    }
    ...
    public void wapPay(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        h5_info.setWap_url("https://gitee.com/javen205/modou");
        h5_info.setWap_name("modou VIP 充值");
        ...
        Map<String, String> params = UnifiedOrderModel
                ...
                .nonce_str(com.modou.core.kit.WxPayKit.generateStr())
                .body("modou 让支付触手可及-H5支付")
                .attach("Node.js 版:https://gitee.com/javen205/TNWX")
                .out_trade_no(com.modou.core.kit.WxPayKit.generateStr())
                .total_fee("1000")
                .spbill_create_ip(ip)
                ...
                .build()
                .createSign(wxPayApiConfig.getPartnerKey(), com.modou.core.enums.SignType.HMACSHA256);

        String xmlResult = WxPayApi.pushOrder(false, params);
        log.info(xmlResult);

        Map<String, String> result = com.modou.core.kit.WxPayKit.xmlToMap(xmlResult);

        String return_code = result.get("return_code");
        String return_msg = result.get("return_msg");
        if (!com.modou.core.kit.WxPayKit.codeIsOk(return_code)) {
            throw new RuntimeException(return_msg);
        }
        String result_code = result.get("result_code");
        if (!com.modou.core.kit.WxPayKit.codeIsOk(result_code)) {
            throw new RuntimeException(return_msg);
        }
    ...
    public AjaxResult webPay(HttpServletRequest request, @RequestParam("total_fee") String totalFee) {
        ...
        Map<String, String> resultMap = com.modou.core.kit.WxPayKit.xmlToMap(xmlResult);
        String returnCode = resultMap.get("return_code");
        String returnMsg = resultMap.get("return_msg");
        if (!com.modou.core.kit.WxPayKit.codeIsOk(returnCode)) {
            return new AjaxResult().addError(returnMsg);
        }
        String resultCode = resultMap.get("result_code");
        if (!com.modou.core.kit.WxPayKit.codeIsOk(resultCode)) {
            return new AjaxResult().addError(returnMsg);
        }
        ...
        Map<String, String> packageParams = com.modou.core.kit.WxPayKit.prepayIdCreateSign(prepayId, wxPayApiConfig.getAppId(),
                wxPayApiConfig.getPartnerKey(), com.modou.core.enums.SignType.HMACSHA256);
    ...
    public AjaxResult scanCode1(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            //获取扫码支付（模式一）url
            String qrCodeUrl = com.modou.core.kit.WxPayKit.bizPayUrl(config.getPartnerKey(), config.getAppId(), config.getMchId(), productId);
            log.info(qrCodeUrl);
            //生成二维码保存的路径
            String name = "payQRCode1.png";
            log.info(ResourceUtils.getURL("classpath:").getPath());
            boolean encode = com.modou.core.kit.QrCodeKit.encode(qrCodeUrl, BarcodeFormat.QR_CODE, 3, ErrorCorrectionLevel.H,
                    "png", 200, 200,
    ...
    public String scanCodeNotify(HttpServletRequest request, HttpServletResponse response) {
        try {
            String result = com.modou.core.kit.HttpKit.readData(request);
            log.info("scanCodeNotify>>>" + result);
            /**
             * 获取返回的信息内容中各个参数的值
             */
            Map<String, String> map = com.modou.core.kit.WxPayKit.xmlToMap(result);
            ...
            String packageSign = com.modou.core.kit.WxPayKit.createSign(packageParams, wxPayApiConfig.getPartnerKey(), SignType.MD5);
             ...
             * 发送信息给微信服务器
             */
            Map<String, String> payResult = com.modou.core.kit.WxPayKit.xmlToMap(xmlResult);
            String returnCode = payResult.get("return_code");
            String resultCode = payResult.get("result_code");
            if (com.modou.core.kit.WxPayKit.codeIsOk(returnCode) && com.modou.core.kit.WxPayKit.codeIsOk(resultCode)) {
                // 以下字段在 return_code 和 result_code 都为 SUCCESS 的时候有返回
                ...
                prepaySign = com.modou.core.kit.WxPayKit.createSign(prepayParams, wxPayApiConfig.getPartnerKey(), com.modou.core.enums.SignType.HMACSHA256);
                prepayParams.put("sign", prepaySign);
                String xml = com.modou.core.kit.WxPayKit.toXml(prepayParams);
                log.error(xml);
                return xml;
            }
        } catch (Exception e) {
    ...
    public AjaxResult scanCode2(HttpServletRequest request, HttpServletResponse response,
        ...
        boolean encode = com.modou.core.kit.QrCodeKit.encode(qrCodeUrl, BarcodeFormat.QR_CODE, 3, ErrorCorrectionLevel.H, "png", 200, 200,
    ...
    public AjaxResult appPay(HttpServletRequest request) {
        ...
        Map<String, String> packageParams = com.modou.core.kit.WxPayKit.appPrepayIdCreateSign(wxPayApiConfig.getAppId(), wxPayApiConfig.getMchId(), prepayId,
                wxPayApiConfig.getPartnerKey(), com.modou.core.enums.SignType.HMACSHA256);
    ...
    public AjaxResult miniAppPay(HttpServletRequest request) {
        ...
        // 以下字段在 return_code 和 result_code 都为 SUCCESS 的时候有返回
        String prepayId = result.get("prepay_id");
        Map<String, String> packageParams = com.modou.core.kit.WxPayKit.miniAppPrepayIdCreateSign(wxPayApiConfig.getAppId(), prepayId,
                wxPayApiConfig.getPartnerKey(), com.modou.core.enums.SignType.HMACSHA256);
        String jsonStr = JSON.toJSONString(packageParams);
    ...
    public String transfer(HttpServletRequest request, @RequestParam("openId") String openId) {
        ...
        Map<String, String> params = TransferModel.builder()
                ...
                .nonce_str(com.modou.core.kit.WxPayKit.generateStr())
                .partner_trade_no(com.modou.core.kit.WxPayKit.generateStr())
                .openid(openId)
                .check_name("NO_CHECK")
                .amount("100")
                .desc("modou 让支付触手可及-企业付款")
                .spbill_create_ip(ip)
                .build()
        ...
        Map<String, String> map = com.modou.core.kit.WxPayKit.xmlToMap(transfers);
    ...
    public String getPublicKey() {
        try {
            ...
            String createSign = com.modou.core.kit.WxPayKit.createSign(params, wxPayApiConfig.getPartnerKey(), SignType.MD5);
    ...
    public String payBank() {
        try {
            ...
            //收款方银行卡号
            params.put("enc_bank_no", com.modou.core.kit.RsaKit.encryptByPublicKeyByWx("银行卡号", PUBLIC_KEY));
            //收款方用户名
            params.put("enc_true_name", com.modou.core.kit.RsaKit.encryptByPublicKeyByWx("银行卡持有人姓名", PUBLIC_KEY));
            //收款方开户行
            params.put("bank_code", "1001");
            params.put("amount", "1");
            params.put("desc", "modou 让支付触手可及-付款到银行卡");
            params.put("sign", com.modou.core.kit.WxPayKit.createSign(params, wxPayApiConfig.getPartnerKey(), com.modou.core.enums.SignType.HMACSHA256));
    ...
    public String queryBank(@RequestParam("partner_trade_no") String partnerTradeNo) {
        try {
            ...
            params.put("nonce_str", System.currentTimeMillis() + "");
            params.put("sign", com.modou.core.kit.WxPayKit.createSign(params, wxPayApiConfig.getPartnerKey(), SignType.MD5));
    ...
    public String refund(@RequestParam(value = "transactionId", required = false) String transactionId,
        ...
        try {
            ...
            Map<String, String> params = RefundModel.builder()
                    ...
                    .out_refund_no(com.modou.core.kit.WxPayKit.generateStr())
                    .total_fee("1")
                    .refund_fee("1")
    ...
    public String refundNotify(HttpServletRequest request) {
        String xmlMsg = com.modou.core.kit.HttpKit.readData(request);
        log.info("退款通知=" + xmlMsg);
        Map<String, String> params = com.modou.core.kit.WxPayKit.xmlToMap(xmlMsg);

        String returnCode = params.get("return_code");
        // 注意重复通知的情况，同一订单号可能收到多次通知，请注意一定先判断订单状态
        if (com.modou.core.kit.WxPayKit.codeIsOk(returnCode)) {
            String reqInfo = params.get("req_info");
            String decryptData = com.modou.core.kit.WxPayKit.decryptData(reqInfo, WxPayApiConfigKit.getWxPayApiConfig().getPartnerKey());
            ...
            return com.modou.core.kit.WxPayKit.toXml(xml);
        }
        return null;
    }
    ...
    public String sendRedPack(HttpServletRequest request, @RequestParam("openId") String openId) {
        try {
            ...
            Map<String, String> params = com.modou.wxpay.model.SendRedPackModel.builder()
                    .nonce_str(com.modou.core.kit.WxPayKit.generateStr())
                    .mch_billno(com.modou.core.kit.WxPayKit.generateStr())
                    .mch_id(wxPayApiConfig.getMchId())
                    .wxappid(wxPayApiConfig.getAppId())
                    .send_name("modou 红包测试")
                    .re_openid(openId)
                    .total_amount("1000")
                    .total_num("1")
                    .wishing("感谢您使用 modou")
    ...
    public String payNotify(HttpServletRequest request) {
        ...
        if (com.modou.core.kit.WxPayKit.verifyNotify(params, WxPayApiConfigKit.getWxPayApiConfig().getPartnerKey(), com.modou.core.enums.SignType.HMACSHA256)) {
            if (com.modou.core.kit.WxPayKit.codeIsOk(returnCode)) {
                // 更新订单信息
                // 发送通知等
                Map<String, String> xml = new HashMap<String, String>(2);
